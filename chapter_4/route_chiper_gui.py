...
def validate_col_row(cipherlist):
    ...
    for i in range(2, len_cipher): # range excludes 1-column cipher
        if len_cipher % i == 0:
            factors.append(i)
    ...
    if ROWS * COLS != len_cipher:
        print("\nError - Input columns and rows not factors and length \
of cipher. Terminating progarm")
        sys.exit(1)
...
def key_to_int(key):
    ...
    if len(key_int) != COLS or key_int_lo < -COLS or key_int_hi > COLS or 0 in key_int:
        print(f"Error - Problem with key. Terminating.""")
        sys.exit(1)
    else:
        return key_int
...
def build_matrix(key_int, cipherlist):
    ...
    for k in key_int:
        ...
        elif k > 0: # read top-to-bottom of colmn
